...
public final class Parser {
     ...
     */
    static void startElement(@NonNull final String tag, @NonNull final Map<String, String> attributes, @NonNull State state) {
        ...
        switch (tag) {
            ...
            case "waitkey":
                if (state.mHtmlType == State.HtmlType.HTML_ADRIFT) {
                    ...
                    if (!Bebek.isStopping()) {
                        GLKController.glk_request_char_event(m, state.mAttachedWindow.getStreamId());
                        GLKEvent ev = new GLKEvent();
                        try {
                            do {
                                GLKController.glk_select(m, ev);
            ...
            case "cls":
                if (state.mHtmlType == State.HtmlType.HTML_ADRIFT) {
                    GLKController.glk_window_clear(m, state.mAttachedWindow.getStreamId());
                }
                break;
            case "c":
                if (state.mHtmlType == State.HtmlType.HTML_ADRIFT) {
