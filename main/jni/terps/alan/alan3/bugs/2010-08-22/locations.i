...
THE floor ISA room_object
	...
	VERB look_in, look_under, look_through, look_behind
...
THE walls ISA room_object
	...
	VERB examine DOES ONLY 
		"You notice nothing unusual about the walls." 
	END VERB.
...
--	IF main_power_switch IS 'on'
--		THEN MAKE d1 lit.
--	END IF.
-- END FOR.

-- FOR EACH makes a loop that affects all instances of the class described after the ISA statement.
-- The 'd1' is a temporary variable needed in the FOR EACH phrase and not anywhere else in the game code.
-- You can name this variable in any way you wish.
-- It is needed to refer to each instance (in 'THEN MAKE d1 lit), because FOR EACH affects all
-- *instances* of the defined class, not the class itself.
...
ADD TO EVERY LOCATION
	...
	ENTERED 
		IF CURRENT ACTOR = hero
			THEN INCREASE visited OF THIS BY 1.
			     INCREASE described OF THIS BY 1.   -- this value also increases after 'look' (see 'verbs.i'.).
		END IF.	 
END ADD TO.						


-- A location not visited at all has the 'visited' value 0. When the hero enters it 
-- the first time, the 'visited' value will change to 1. On the second visit the value will be 2, etc. 
...
-- will increase when the NPC enters the location, as well. This is most often not what is wanted,
-- and that's why the 'if' statement (IF CURRENT ACTOR = hero) is included above. 


-- You can also check whether the hero has been in a location if need be:
...
-- that's why we identify it here as 'current_room'. Through the NAME statement, it can be still 
