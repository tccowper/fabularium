...
--------------------------------------------------------------------
	...
	ELSIF wear_flag OF hero = 1 
		THEN
			"You don't feel like putting on" SAY THE THIS. "."
	ELSE
		"You don't feel like putting on" SAY THE THIS. "."
	END IF.
...
EVERY door ISA OBJECT
	...
	VERB close
		DOES ONLY "It wouldn't serve any purpose to close the door."
	END VERB.
...
-- be 'no_vessel' (= the dummy default container to be ignored):
...
EVERY window ISA OBJECT
	...
	VERB examine
		DOES ONLY 
			"The window is boarded"
				IF house IS 'entered'
					THEN  "and only lets a little light into the room"
				END IF.
				"."
			IF hero AT lr
				THEN
			IF kitchen_window IS closed
				THEN 
					"$p$p(The Kitchen)$nThe window is boarded and only lets a little light into the room."
				ELSE 
	...
	VERB look_out_of
		DOES ONLY "You can't look out of the boarded window."
			IF hero AT lr THEN "$p$p(The Kitchen)$nYou can't look out of the boarded window." END IF.
	END VERB.


	VERB look_through
		DOES ONLY "You can't see through the boarded window."
			IF hero AT lr THEN "$p$p(The Kitchen)$nYou can't see through the boarded window." END IF.
	END VERB.

	VERB open
		DOES ONLY "You can't open the boarded window."
			IF hero AT lr THEN "$p$p(The Kitchen)$nYou can't open the boarded window." END IF.
	END VERB.
...
THE hero ISA ACTOR
	...
	IS NOT sitting.
	IS NOT lying_down.
	HAS i_value 0. -- for inventory message
	...
	------------------------------------
  	...
  	HEADER
		...
		END IF.
    	ELSE
		IF CURRENT LOCATION ISA SITE
			THEN "You are carrying nothing."
				IF i_value OF hero = 0
					THEN
				 "That's how you like to start."
					INCREASE i_value OF hero. 
				END IF.
	...
	VERB examine
		DOES ONLY "You're Ted Paladin, text adventure hero. 
			     You like to think there is no house implemented you can't get in or out of,
			     and so far that has held true. You have your reputation to defend, so you'll have to
			     take care not to spoil things this time either."
				IF hero AT lr
					THEN
				"$p$p(The Kitchen)$n"
				"You're Ted Paladin, text adventure hero. 
			     You like to think there is no house implemented you can't get in or out of,
			     and so far that has held true. You have your reputation to defend, so you'll have to
			     take care not to spoil things this time either."
				END IF.
...
SYNONYMS
  me, myself, yourself, self, ted, paladin = hero.
