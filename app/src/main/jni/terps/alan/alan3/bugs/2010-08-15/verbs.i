...
ADD TO EVERY THING
  VERB ask
	...
	AND act IS reachable			-- you might want to remove this check in some situations, e.g.
							-- when an NPC (= a non-player character) is speaking on the phone with the hero
...
ADD TO EVERY LIQUID
  VERB drink
	...
	DOES
		IF obj NOT IN hero
			THEN "(taking" SAY THE vessel OF obj. "first)$n"
				LOCATE vessel OF obj IN hero.
		END IF.
...
VERB 'look'
	DOES
		LOOK.
		INCREASE described OF CURRENT LOCATION. 		-- see 'locations.i', attribute 'described'.
END VERB.
...
VERB pour
	...
	DOES
		...
		"You pour the contents of" SAY THE obj. "onto"
			IF floor HERE 
				THEN "on the floor."
				ELSE "on the ground."
			END IF.
...
SYNTAX pour_on = pour (obj1) 'on' (obj2)
		WHERE obj1 ISA LIQUID
			ELSE "You can only pour liquids."
		AND obj2 ISA OBJECT
			ELSE 
				IF obj2 ISA ACTOR
					THEN 
						...
						END IF.
					ELSE "You can't $v anything onto" SAY THE obj2. "."
				END IF.
		AND obj2 ISA CONTAINER
   ...
   WHEN obj1
	...
	DOES
		...
		IF obj2 = floor OR obj2 = ground
			...
			"You pour the contents" SAY THE obj1. "on" SAY THE obj2. "."
...
ADD TO EVERY OBJECT
	...
	WHEN obj1	
	    ...
	    DOES
		 ...
		 -- (+ you would need to define some attributes to check that the object is behind another	
		 -- object, etc.)
...
-- To use this verb in the meaning 'wear', see the file 'classes.i', class 'clothing', verb 'wear'.

-- You can put things on the floor/ground or on a supporter. In other cases, the response will be
-- "That wouldn't accomplish anything."
...
ADD TO EVERY THING
	...
	AND CURRENT LOCATION IS lit
		ELSE 
			IF obj = hero
				THEN "Nothing would be achieved by that."  -- you can squeeze yourself in the dark, as well
				ELSE "It is too dark to see."
			END IF.
	AND obj IS reachable
			ELSE SAY THE obj. "is out of your reach."		
...
ADD TO EVERY OBJECT
  VERB take
	CHECK CURRENT LOCATION IS lit
    	...
    	DOES
		IF obj DIRECTLY IN hero			-- i.e. the object to be taken is carried by the hero already						
			THEN "You already have" SAY THE obj. "."  	
		ELSIF obj DIRECTLY IN worn		-- i.e. the object to be taken is a piece of clothing that the player character is wearing;
							-- here, this verb works in practise like 'take off'. 
			THEN "You take off" SAY THE obj. "and carry it in your hands."
				LOCATE obj IN hero.		
		ELSE "Taken."				-- this covers also cases where the object to be taken is in another container, 
			LOCATE obj IN hero.		-- such as a wallet that is in a jacket the hero is carrying or wearing.
		END IF.			
  END VERB.
END ADD TO.
...
ADD TO EVERY OBJECT
  VERB take_from
	...
	AND obj1 IS takeable
      	ELSE 
			IF obj1 ISA SCENERY
				THEN "Unimportant for your purposes, you leave the" SAY THE obj1. "where it is."
				ELSE "That's not something you can take."
			END IF.
    ...
    WHEN obj1
	    ...
	    DOES 
		  	...
		  	END IF.
      	  	...
      	  	IF obj2 IS inanimate
				THEN "The" SAY obj1. "bounces harmlessly off" SAY THE obj2. "and ends up on the"
		  				IF floor HERE
