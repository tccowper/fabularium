...
EVERY liquid ISA OBJECT
	...
	CONTAINER				-- We declare this subclass a container to enable player commands
						-- such as 'throw sack in water', 'take pearl from water' and 'look into
						-- water'. Also cases such as 'pour blue potion in red potion' require
						-- that the liquid class behaves like a container.
	...
	-- If  you have some liquid in a container in your game, you should declare the liquid thus:
	
 	-- THE juice ISA LIQUID
	--      IN bottle
	-- 	  HAS vessel bottle.	-- i.e. the value of the 'vessel' attribute is the container
	-- 	  ...				-- the liquid is in
	-- END THE juice.


	-- Then, taking and pouring liquids work smoothly.
	-- If you leave the 'vessel' attribute out, taking a liquid will yield "You can't carry
	-- [the liquid] around in your bare hands."; only taking its container will work.
...
-- We still declare the default vessel for liquids; if the vessel of a liquid is
...
-- Here is the event checking that if a liquid is at any time outside a container,
-- its 'vessel' attribute will be 'zero_vessel':


EVENT check_vessel
	FOR EACH liq ISA LIQUID DO
		IF vessel OF liq <> zero_vessel
			THEN 
				IF COUNT ISA LIQUID, IN vessel OF liq = 0
					THEN SET vessel OF liq TO zero_vessel.
				END IF.
		END IF.
	END FOR.
	SCHEDULE check_vessel AFTER 1.
...
-- see checks for this subclass in the file 'verbs.i', verbs 'attack_with', 'fire',
